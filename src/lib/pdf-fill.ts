...
import { matchPdfFieldByName } from "@/lib/pdf-fill-client";
import { fitDimensionsInBox } from "@/lib/signature";
...
export async function fillPdfDocument(options: {
  ...
  for (const item of filledFields) {
    const mapping = mappings.find((entry) => entry.label === item.label);
    if (!mapping) continue;
    const field = matchPdfFieldByName(fields, mapping.pdf_field);
  ...
  try {
    ...
    // Some PDFs lack embedded fonts — continue without appearance update.
  }
  ...
  if (signatureDataUrl) {
    await embedSignature(doc, signatureDataUrl, signaturePlacement);
  }
